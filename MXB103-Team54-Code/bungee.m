...
fprintf('Task 2: Max velocity %.4f m/s at %.4f seconds\n\n', maxV, timemaxV);

% Task 3
[accel_max, accel_max_t] = maxacceleration(v, t, h, n);
fprintf('Task 3: The max acceleration is %.4f m/s^2 and occurs at %.4f s after the jump\n\n', accel_max, accel_max_t);

% Task 4
distance = integration_simprule (v, h, n);
fprintf('Task 4: The total distance travelled during the 60 second jump is %.4f m\n\n', distance);

% Task 5
camera_time = task5_camera(y, t);
fprintf('Task 5: To take a photo of the jumper, the camera should be set to trigger at %.4f s after the jump\n\n', camera_time)

% Task 6
[dis_from_water_original_jump, dis_from_water_mod_jump, new_k, new_L, max_accel] = modified_bungee(y, t);
fprintf('Task 6: Originally, at max displacement from the jump point the jumper misses the water by %.4f m.\n', dis_from_water_original_jump)
fprintf('But with the length of the rope modified to %d m, and its spring constant to %d N/m,\n', new_L, new_k)
fprintf('The jumper now touches the water, reaching %.4f m into it.\n', dis_from_water_mod_jump*-1)
fprintf('The max acceleration of the jumper also remains just under 2g, with it reaching up to %.4f m/s^2\n\n', max_accel)
